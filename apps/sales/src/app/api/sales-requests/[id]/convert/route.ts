...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return runWithRequestContextAsync(reqCtx, async () =>
    ...
    const sreq = await prisma.salesRequest.findFirst({
      where: { id, tenantId },
      include: { items: true, customer: true },
    });
    if (!sreq) return NextResponse.json({ error: "Sales request not found" }, { status: 404 });
    if (sreq.status !== "OPEN") {
      return NextResponse.json({ error: `Request is ${sreq.status}, cannot convert` }, { status: 409 });
    }
    ...
    if (sreq.customer.isBlocked) {
      return NextResponse.json(
        { error: `Customer is blocked: ${sreq.customer.blockedReason ?? "contact support"}` },
        { status: 403 }
      );
    }
    ...
    try {
      ...
      await notifyPortalCustomer({
        ...
        body: `${sreq.requestNumber} converted to ${result.order.orderNumber}. Fulfillment has started.`,
        metadata: {
          salesRequestId: sreq.id,
          requestNumber: sreq.requestNumber,
          orderId: result.order.id,
          orderNumber: result.order.orderNumber,
        },
      });
