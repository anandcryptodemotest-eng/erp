...
export function TaskScreenRuntime(props: {
  ...
  hostApis?: HostApis;
  /** When true, skip auto-bootstrap (caller already ran bootstrapAdminRuntime). */
  skipBootstrap?: boolean;
}): ReactNode {
  if (!props.skipBootstrap) {
  ...
  if (!screen) {
    return (
      <div className="rounded-xl border-2 border-amber-400 bg-amber-50 p-4 text-sm text-amber-900">
        Form has no layout. Add Form Fields and Action Buttons, then save.
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div>
        ...
        {screen.description && (
          <p className="text-xs mt-0.5 text-[var(--ink-soft)]">{screen.description}</p>
        )}
      </div>
