...
const FormFieldsWidget: Widget = {
  render(runtime: UIRuntime) {
    ...
    function renderControl(
      ...
      const cls =
        opts?.className ??
        "mt-0.5 w-full rounded border border-[var(--line)] px-2 py-1.5 text-sm disabled:bg-[var(--mist)]";
      if (f.type === "readonly") {
        return <span className="mt-0.5 block text-sm text-[var(--ink-soft)]">{value || "—"}</span>;
      }
      if (f.type === "textarea") {
        return (
          <textarea
        ...
        if (f.type === "radio") {
          return (
            <div className="mt-1 flex flex-wrap gap-3">
              {options.map((o) => (
                <label key={o.value} className="flex items-center gap-1.5 text-sm text-[var(--ink-soft)]">
                  <input
                    type="radio"
                    name={f.key}
                    disabled={!canEdit}
      ...
      <div className="space-y-3" key="FormFields">
        ...
        {perItem.length > 0 &&
          items.map((item) => (
            <div
              key={item.id}
              className="rounded-lg border border-[var(--line)] bg-[var(--mist)]/50 p-3 text-sm space-y-2"
            >
              <div className="flex items-center justify-between gap-3">
                <span className="font-medium text-[var(--ink)]">{item.productName}</span>
              ...
              <div className="flex flex-wrap items-end gap-3">
                {perItem.map((f) =>
                  f.type === "readonly" ? (
                    <span key={f.key} className="text-xs text-[var(--ink-soft)] pb-2">
                      {f.label}{" "}
                      {String(
                    ...
                    <label key={f.key} className="block text-xs text-[var(--ink-soft)]">
                      ...
                      {renderControl(
                        ...
                        {
                          className:
                            "mt-0.5 block w-32 rounded border border-[var(--line)] px-2 py-1.5 text-sm disabled:bg-[var(--mist)]",
                        }
                      )}
                    </label>
                  )
            ...
            {editable && runtime.context.permissions.canEdit ? (
              <div className="flex flex-wrap items-end gap-3">
                <label className="block text-xs text-[var(--ink-soft)]">
                  Qty
                  <input
                    className="mt-0.5 block w-28 rounded border border-[var(--line)] px-2 py-1.5 text-sm"
                    type="number"
                    min={1}
                    value={
                ...
                {canEditPrice ? (
                  <label className="block text-xs text-[var(--ink-soft)]">
                    Unit price
                    <input
                      className="mt-0.5 block w-32 rounded border border-[var(--line)] px-2 py-1.5 text-sm"
                      type="number"
                      min={0}
                      step="0.01"
                      value={
                  ...
                  <span className="text-xs text-[var(--ink-soft)] pb-2">Unit price {item.unitPrice}</span>
                ) : null}
              </div>
            ) : (
              <span className="text-[var(--ink-soft)]">
...
function CatalogSearchInner({ runtime }: { runtime: UIRuntime }) {
  ...
  return (
    <div className="relative space-y-2 rounded-lg border border-dashed border-[var(--line)] p-3">
      <p className="text-xs font-medium text-[var(--ink-soft)]">Add product</p>
      <input
      ...
      />
      {busy && <p className="text-[11px] text-[var(--ink-soft)]">Searching…</p>}
      {hits.length > 0 && (
        <ul className="max-h-40 overflow-y-auto rounded-lg border border-[var(--line)] bg-white divide-y divide-[var(--line)]">
          {hits.map((p) => (
            <li key={p.id}>
              <button
                type="button"
                className="w-full px-3 py-2 text-left text-sm hover:bg-[var(--mist)]"
                onClick={() => {
                  editor!.addProduct(p);
                  setQ("");
                  setHits([]);
              ...
              >
                ...
                {p.sku && <span className="ml-2 text-xs text-[var(--ink-soft)]">{p.sku}</span>}
                {p.sellPrice != null && (
                  <span className="block text-xs text-[var(--ink-soft)]">{p.sellPrice}</span>
                )}
              </button>
            </li>
          ))}
